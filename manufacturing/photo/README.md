...
docker build \
  ...
  --build-arg IMAGE=photo:1.0.0 \
  --build-arg TEXT_COLOR=#FFFFFF \
  --build-arg BG_COLOR=#2C3E50 \
  -t photo:1.0.0 .

# Run
docker run -p 8080:8080 photo:1.0.0
```
...
```bash
helm install photo ./deployments/helm/photo \
  --set image.tag=1.0.0

helm upgrade photo ./deployments/helm/photo \
  --set image.tag=1.1.0

helm uninstall photo
```
